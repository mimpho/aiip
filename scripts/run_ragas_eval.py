"""RAGAS Faithfulness + Answer Relevancy contra el pipeline real (E-07 T-02).
...
tests/eval/e07_t02_ragas_faithfulness_relevancy.feature.

La ejecución es incremental: cada caso ya presente en
tests/eval/results/e07_t02_ragas_scores.json se salta, así que una
interrupción a mitad de ejecución (p. ej. 429 de cuota de Gemini) no pierde
el progreso ya hecho ni repite llamadas.
...
def main() -> None:
    ...
    informative_cases = [c for c in all_cases if not c.is_alarm]
    print(f"Casos informativos a evaluar: {len(informative_cases)}")
    ...
    for case in informative_cases:
        ...
        if has_alarm:
            # Caso informativo (is_alarm=false en el dataset) que disparó el
            # filtro de seguridad de forma inesperada: no es un bug de este
            # script, pero es una señal relevante para el informe de T-04.
